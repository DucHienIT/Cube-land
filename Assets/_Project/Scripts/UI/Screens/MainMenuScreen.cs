...
namespace CubeBlaster
{
    public class MainMenuScreen : UIScreen
    {
        ...
        {
            MakeDecoCube(parent, deco[0], new Vector2(0f, 0f), new Vector2(100, 300), 120, -12f, 0.0f);
            MakeDecoCube(parent, deco[1], new Vector2(1f, 0f), new Vector2(-100, 480), 96, 9f, 1.3f);
            MakeDecoCube(parent, deco[2], new Vector2(0f, 1f), new Vector2(110, -340), 84, 14f, 2.1f);
            MakeDecoCube(parent, deco[3], new Vector2(1f, 1f), new Vector2(-90, -900), 108, -8f, 3.0f);
            MakeDecoCube(parent, deco[4], new Vector2(0f, 0.5f), new Vector2(70, -260), 70, 20f, 4.2f);
            MakeDecoCube(parent, deco[0], new Vector2(1f, 0.5f), new Vector2(-80, 120), 76, -18f, 5.1f);
        }
        ...
        void MakeDecoCube(Transform parent, Color color, Vector2 anchor, Vector2 position,
            float size, float tilt, float phase)
        {
            var go = new GameObject("Deco", typeof(RectTransform), typeof(Image));
            go.transform.SetParent(parent, false);
            UIFactory.Rect(go, anchor, anchor,
                new Vector2(position.x - size * 0.5f, position.y - size * 0.5f),
                new Vector2(position.x + size * 0.5f, position.y + size * 0.5f));
